  ...
  namespace server {
    namespace status_strings {
      ...
      const std::string service_unavailable =
			...
			boost::asio::const_buffer to_buffer(StatusCode status) {
				switch(status) {
				case StatusCode::ok:
		...
		namespace misc_strings {
			const char name_value_seprarator[] = {':', ' '};
			const char crlf[] = {'\r', '\n'};
		} // namespace misc_strings
	...
	buffers.push_back(boost::asio::buffer(misc_strings::name_value_seprarator));
    ...
    namespace stock_responses {
      ...
      const char no_content[] =
				"<html>"
				"<head><title>No Conent</title></head>"
				"<body><h1>204 No Content</h1></body>"
				"</html>";
      const char multiple_choices[] =
				"<html>"
				"<head><title>Multiple ChoicesCreated</title></head>"
    ...
    Response Response::stock_response(StatusCode status) {
      Response response;
      response.status = status;
      response.content = stock_responses::to_string(status);
